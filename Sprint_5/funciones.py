...
def guardar_datos():
    try:
        with open("Mascotas_dueños.csv", "w", newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=["nombre_mascota", "especie", "raza", "nombre_tutor", "telefono", "direccion"])
            writer.writeheader()
            for m in pacientes:
                writer.writerow({
        ...
        logger.info("Datos de mascotas y dueños guardados en archivo CSV.")        
        print("Datos de mascotas y dueños guardados en archivo CSV.")


        data = {} #lista vacia para json
        # for m in pacientes:
        #     data[m.nombre_paciente] = [{
        #         "fecha": c.fecha,
        #         "motivo": c.motivo,
        #         "diagnostico": c.diagnostico
        #     }for c in m.consultas] #version corta de guardar resultado.
        ...
        logger.info("Datos de consultas guardados en archivo JSON.")
        print("Datos de consultas guardados en archivo JSON.")
